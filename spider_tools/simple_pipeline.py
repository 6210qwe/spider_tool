...
_DEFAULT_PROMPTS = {
    "img2markdown": {
        ...
        "user_prompt": """请 请将图像内容转换为规范的纯文本格式文本。
    
    要求：
    1. **仅提取图像中清晰、可辨、以印刷体呈现的文字内容**。手写体、示意图、流程图、设备结构图、曲线图、照片、图标、装饰性元素等非文本内容一律忽略，即使其中包含零星文字标签也不得提取。
    2. 忠实还原原始文档的逻辑结构：包括章节标题、段落、编号列表、项目符号、表格（用纯文本对齐表示）、代码块（识别语言并用 ```language 包裹）、数学公式（必须用 LaTeX，如 $ ... $ 或 
    $$
     ... 
    $$
    ）。
    3. 对因图像质量问题导致的模糊、断裂、重影、墨渍等，可基于上下文和行业常识进行**最小必要修复**（例如：“GB/T 1688˜” → “GB/T 16886”），但禁止无依据推测。
    4. 长行文本若因页面宽度在图像中被视觉折行（无连字符且语义连续），应合并为单行；仅在原文存在空行、列表项、表格行结束、章节分隔等**明确结构信号**时才换行。
    5. 若公式末尾有省略号（如“…”或“⋯”），最多保留一个标准 LaTeX 省略命令（如 \ldots 或 \cdots），禁止重复堆砌。
    6. 若整页无有效印刷体文本（例如仅为图示页、封面图、空白页），输出：“(空白页)”。
    7. **绝对禁止**：
       - 描述图像布局（如“左侧有一个流程图”）；
       - 解释图表含义；
       - 补充标准编号、条款内容或常识性定义；
       - 输出任何不属于原文的说明、前缀、后缀或元信息（如“转换结果：”）。
    8. 所有输出必须是**可直接用于合规文档归档的纯文本**，格式严谨、无冗余、无幻觉。  
""",
